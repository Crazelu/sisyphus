...
class CandleInfoText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (data.length - 7 > currentIndex) {
      final list =
          data.sublist(currentIndex, currentIndex + 6).map((e) => e.close);
      ma7 = (list.fold<double>(0, (double p, double c) => p + c)) / list.length;
    }

    if (data.length - 25 > currentIndex) {
      final list =
          data.sublist(currentIndex, currentIndex + 24).map((e) => e.close);
      ma25 =
          (list.fold<double>(0, (double p, double c) => p + c)) / list.length;
    }

    if (data.length - 99 > currentIndex) {
      final list =
          data.sublist(currentIndex, currentIndex + 98).map((e) => e.close);
      ma99 =
          (list.fold<double>(0, (double p, double c) => p + c)) / list.length;
    }
    ...
    return Column(
      ...
      children: [
        RichText(
          text: TextSpan(
            text: symbol, //dateFormatter(candle.date),
            style: TextStyle(
